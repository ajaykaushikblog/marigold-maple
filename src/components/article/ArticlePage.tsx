...
import { Pinterest, Facebook, Share, Clock } from '../ui/icons'
...
function ShareRow({ title }: { title: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="mr-1 text-[0.72rem] font-bold uppercase tracking-[0.14em] text-muted-foreground">Share</span>
      <button className={`${btn} !bg-[#e60023] !text-white !border-transparent`} aria-label="Save to Pinterest">
        <Pinterest width={16} height={16} />
      </button>
      <a href="#" className={btn} aria-label="Share on Facebook"><Facebook width={16} height={16} /></a>
      <a href="#" className={btn} aria-label="Share on X"><XIcon /></a>
      <a href={`mailto:?subject=${encodeURIComponent(title)}`} className={btn} aria-label="Share by email">
      ...
      <button onClick={copy} className={btn} aria-label="Copy link">
        {copied ? '✓' : <Share width={15} height={15} />}
      </button>
    </div>
  )
}
